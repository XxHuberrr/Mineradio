...
### 2026-07-13 - 永久本地曲库、本地歌单与歌词三态

- 用户认可/要求保留：本地音乐只记录原文件夹路径和索引，不复制音频；导入多个文件夹时持续合并，重启后仍保留。游客也能创建本地歌单，本地歌单可混合本地、QQ、网易云歌曲。
- 涉及文件：`desktop/local-music-library.js`、`desktop/local-library-store.js`、`desktop/main.js`、`desktop/preload.js`、`public/local-library-core.js`、`public/lyric-display-core.js`、`public/index.html`。
- 关键参数/实现：本地索引保存于 Electron `userData/local-library-v1.json`；失效文件保留记录；搜索优先级固定为本地 > QQ > 网易云并自动去重；歌词按钮固定为“词（关）/词（开）/词（滚）”，括号状态使用右下角小字；`lyricDoubleLine` 控制 `词（开）`是否显示当前句和下一句；`词（滚）`固定维护当前句和下一句两个网格，切句复用旧下一句并最多替换一个新网格。
- 禁止回退或改坏的点：不要恢复导入新文件夹覆盖旧队列/曲库；不要复制或移动用户音乐文件；删除歌单不得删除磁盘歌曲；游客或未登录状态不得自动弹出平台登录；不要在每次歌词切句时清空并重建全部滚动网格，也不要破坏 3D 歌单详情页歌词层级。
