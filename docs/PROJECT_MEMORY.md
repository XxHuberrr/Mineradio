...
- 关键参数/实现：本地索引保存于 Electron `userData/local-library-v1.json`；失效文件保留记录；搜索优先级固定为本地 > QQ > 网易云并自动去重；歌词按钮固定为“词（关）/词（开）/词（滚）”，括号状态使用右下角小字；滚动歌词窗口固定 7 个时间节点；译文精确容差 `0.35s`，动态最大容差 `3.5s`，匹配保持单调且不重复使用译文行。
- 禁止回退或改坏的点：不要恢复导入新文件夹覆盖旧队列/曲库；不要复制或移动用户音乐文件；删除歌单不得删除磁盘歌曲；游客或未登录状态不得自动弹出平台登录；不要一次性创建整首 3D 歌词对象，也不要破坏 3D 歌单详情页歌词层级。
