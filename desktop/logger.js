/**
 * Mineradio 临时日志模块（主进程）
 *
 * 提供文件级日志写入能力，按日切分，自动清理 7 天前的日志。
 * 由 desktop/main.js require，并在 app.whenReady() 后通过 setLogDir 设置日志目录。
 * 同时通过 IPC 通道 `mineradio-log-write` 接收前端日志。
 *
 * 日志级别由环境变量 MINERADIO_LOG_LEVEL 控制（默认 INFO）：
 *   DEBUG < INFO < WARN < ERROR < OFF
 *
 * 日志格式：
 *   [YYYY-MM-DD HH:mm:ss.SSS] [LEVEL] [scope] message | data(JSON)
 *
 * 文件轮转：
 *   单个日志文件超过 10MB 时，自动切分为 mineradio-YYYY-MM-DD-1.log、-2.log 等。
 *
 * 自动清理：
 *   启动时删除 7 天前的日志文件。
 */
...
function rotateIfNeeded(filePath) {
  try {
    ...
    // 轮转：mineradio-YYYY-MM-DD.log -> mineradio-YYYY-MM-DD-1.log, -2.log ...
