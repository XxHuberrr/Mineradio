...
桌面版支持导入现有的 LX Music Desktop 自定义音源 `.js` 脚本，兼容目标为洛雪自定义源 API `2.0.0`。

1. 点击右上角“源”打开“洛雪自定义音源”。
2. 选择“导入 `.js` 音源”，确认安全提示后选择脚本。
3. 脚本验证通过后，在列表中点击“启用”。一次只能启用一个脚本。
4. 停用音源后，Mineradio 会恢复原有的网易云/QQ 播放地址解析。

Mineradio 的搜索结果仍来自网易云音乐和 QQ 音乐；自定义脚本只参与播放地址解析。导入的脚本只保存在本机，不会随应用打包、上传或同步。第三方脚本可以把歌曲信息及脚本自身持有的凭据发送到任意 HTTP/HTTPS 服务，只应导入你信任的脚本。
