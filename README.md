...
## macOS Apple Silicon 预览构建

Mineradio 现在包含 macOS Apple Silicon 本地构建链路和 macOS 运行时适配。该路径主要用于本地验证、开发测试和后续签名分发准备；正式公开发布仍需要 Apple Developer ID 签名和 notarization。
...
- `dist/Mineradio-<version>-arm64.zip`
- `dist/latest-mac.yml`

如果只需要生成 `.app` 进行本地 smoke test：
...
- 原生 macOS 应用菜单、Dock 激活和 hidden-inset traffic lights 窗口模式。
- 平台感知的更新资源选择，macOS 优先 `.dmg/.zip`，Windows 继续优先 `.exe/.msi`。
- 跨平台节拍缓存目录，避免 macOS 回落到 Windows 路径。
- macOS 桌面歌词和壁纸 overlay fallback。
- macOS 主窗口 compositor 优化和空闲渲染降载。

验证记录：

- [macOS build verification](./docs/MAC_BUILD_VERIFICATION_2026-06-28.md)
- [macOS performance verification](./docs/MAC_PERFORMANCE_VERIFICATION_2026-06-28.md)
...
macOS Apple Silicon 本地构建使用 `npm run build:mac`，会生成 `dist/mac-arm64/Mineradio.app`、`dist/Mineradio-<version>-arm64.dmg`、`dist/Mineradio-<version>-arm64.zip` 和 `dist/latest-mac.yml`。如果本机没有 Apple Developer ID 证书，可用 `CSC_IDENTITY_AUTO_DISCOVERY=false npm run build:mac` 生成本机可验证的未签名包；正式公开分发仍需要 Developer ID 签名和 Apple notarization。

## 更新机制

Mineradio 会请求 GitHub Releases latest 检测新版本。远端版本高于本地版本时，应用内更新入口会展示 Release 内容、下载安装包到本机用户数据目录，并通过系统打开安装包。

本地验证更新链路时，可以通过 `MINERADIO_UPDATE_MANIFEST` 指向一个本地 manifest JSON 或 HTTP 地址来模拟线上 Release。更新资源选择会根据运行平台过滤候选安装包：Windows 优先 `.exe/.msi`，macOS 优先 `.dmg/.zip`。
