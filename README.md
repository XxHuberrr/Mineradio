...
- QQ 音乐搜索、登录态与音源补充接入
- Spotify 搜索、歌单与整曲播放接入（自带 Client ID，需 Premium，详见 [docs/SPOTIFY_SETUP.md](./docs/SPOTIFY_SETUP.md)）
- 多平台账号并排展示，首页「我的歌单」按最近播放来源优先排序
...
## Spotify 支持（自带 Client ID）

Mineradio 的 Spotify 接入采用 **BYO（Bring Your Own）Client ID** 模式：每位用户在 Spotify 开发者后台创建自己的应用，把自己的 Client ID 填进 Mineradio 的登录区域。软件**不内置、也不共享任何 Client ID**，登录走 Authorization Code with PKCE，**不需要 Client Secret**。整曲播放依赖 Spotify Web Playback SDK，**需要 Spotify Premium**。

完整步骤（创建应用、Redirect URI、填入 Client ID、自行打包发布时的 EVS 证书说明）见 [docs/SPOTIFY_SETUP.md](./docs/SPOTIFY_SETUP.md)。

## 第三方音乐平台说明

Mineradio 不是网易云音乐、QQ 音乐、腾讯音乐娱乐集团或 Spotify 的官方客户端，也不隶属于任何音乐平台。

项目中的第三方平台接入仅用于个人学习、本地客户端体验和用户自有账号的播放辅助。请遵守对应平台的用户协议、版权规则和会员权益规则；Spotify 接入还需遵守 Spotify 开发者条款，且由用户使用自有 Client ID 和自有账号。项目不会提供绕过付费、绕过会员、破解音质或重新分发音乐内容的能力。
