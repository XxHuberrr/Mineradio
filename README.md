...
- 本地音乐画像与推荐模式：默认关闭；开启后仅分析本机播放行为，并且可随时清除
...
音乐画像默认关闭。启用后，Mineradio 只在本机保存必要的播放事件和可信的风格、语种、年代维度，用于让你看懂当前偏好；缺少可信元数据时会明确显示“数据不足”，不会从歌名、歌手等文本猜测标签。画像数据可一键清除，清除后推荐模式也会关闭。

使用方法：在首页“音乐画像”卡片启用画像；累计五首不同的有效在线音乐行为后，可在首页或播放器控制区开启“推荐模式”。推荐模式只重排当前临时播放队列中当前歌曲之后的第六首及以后歌曲；接下来的五首和你手动拖动过的歌曲不会被移动。关闭模式会保留当前队列顺序，并停止后续调整。

如果某个标签不符合你的口味，可以在画像卡片点“减少此类推荐”。系统会降低该标签的推荐权重；之后 30 天内，若你对五首不同歌曲产生收藏、正常完播或有效长时收听等正向行为，系统会把该标签恢复到正常权重，并持久保存恢复状态，窗口过期后不会再次自动回落。早期手动切歌指播放进度在前 15%；普通歌曲播放达到 80% 才算完播，长 DJ 曲目达到 50% 算有效长时收听。早切、收藏、完播分别使用 -50、+40、+10 的排序权重，这些是算法权重，不是“喜欢或不喜欢”的概率。

第一期不删除歌曲、不自动补歌、不搜索外部歌曲，也绝不修改、重排或写入你的任何已保存歌单。画像状态不保存或上传 Cookie、播放 URL、歌词、封面和账号凭证；本地音乐与播客不参与第一期画像。

Electron GUI 已完成手工验收：首次启动画像默认为关闭；启用后播放不足五首不同在线音乐时队列不变；有可信元数据的歌曲完成播放或收藏后，卡片只显示已提供维度并标明数据不足；至少七首队列中手动拖动受调整区歌曲并触发早期手动切歌后，接下来五首和被拖动歌曲位置不变；关闭推荐模式并清除画像后，临时队列保持、画像清空且歌单和账户不受影响。

已知基线：当前完整静态检查中的 `tests/local-music-library-persistence.test.js` 仍有独立的 FLAC 标签读取失败（期望“标签标题”，实际为“Song”）。它位于本地音乐库路径，与音乐画像模块无文件交集；本次第一期画像贡献不把它伪装为已修复。
