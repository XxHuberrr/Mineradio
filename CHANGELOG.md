...
- 主窗口增加外站导航拦截，只允许应用自身本地地址在窗口内加载，HTTP(S) 外链改由系统浏览器打开。
- 新增 `npm run check`，统一检查核心 JavaScript、HTML 内联脚本、JSON、合并冲突标记和服务安全边界测试。
- 网易云与 QQ 音乐登录会话在 Electron 中改用系统 `safeStorage` 加密保存，旧明文 Cookie 首次读取后自动迁移，登出时删除对应会话文件。
- 系统安全存储不可用时不再回退为 Electron 明文持久化；当前会话仍可继续使用，并明确提示重启后需要重新登录。
- 新增 Cookie 存储回归测试，覆盖加密读写、明文迁移、文件权限、登出删除、损坏数据和 standalone Node 兼容路径。
- 快速补丁改为强制 Ed25519 非对称签名验证：只有内置可信公钥验证通过的签名信封才能在版本校验和文件写入前继续应用。
- 未签名、被篡改、未知密钥、错误算法或未配置可信公钥的快速补丁会被拒绝，并保留完整安装包更新路径。
- 新增 `npm run sign:patch` 发布签名工具和回归测试；当前生产公钥尚未配置，因此快速补丁默认关闭，待完成正式密钥建立后再启用。
- 快速补丁应用改为全量预检后再写入：提前拒绝非法路径、Windows 保留名、重复目标、错误编码和文件哈希不一致，避免半包覆盖。
- 新增补丁事务回滚：写入过程中任一文件失败时自动恢复已覆盖文件并删除本轮新建文件；回滚不完整会返回独立高风险错误，要求改用完整安装包修复。
- 加固补丁真实文件系统边界：记录应用根目录真实路径，逐级拒绝符号链接、Windows 目录联接和越界解析，并在每个文件写入前重新检查目标路径。
- 补丁失败回滚会从深到浅清理由本任务创建且仍为空的目录；目录清理异常会并入 `PATCH_ROLLBACK_FAILED`，避免留下半完成目录结构。
- 补丁预检会记录目标文件身份；目标在预检后被新增、删除或替换时返回 `PATCH_TARGET_CHANGED`，不覆盖并发产生的本地文件。
- 保留 `.mineradio-patch` 和 `.mineradio-restore` 内部事务后缀，拒绝补丁将其作为目标文件，避免目标与临时/恢复文件发生命名碰撞。
