...
- 本地曲库改为永久多文件夹索引：连续导入新文件夹不会覆盖旧歌曲，重启后自动重新扫描，只保存路径和元数据，不复制音频文件。
- 新增本地歌单：支持创建、重命名、删除、播放全部，以及本地、QQ、网易云歌曲混合保存；删除歌单不会删除磁盘文件。
- 本地曲库支持歌曲单选、全选、反选、取消选择和批量加入歌单，重复歌曲自动跳过。
- 搜索结果根据平台登录状态限制来源；未登录时只搜索本地曲库，综合结果按“本地、QQ、网易云”优先级排序并自动去重。
- 底部歌词按钮新增三态循环：词（关）、词（开）、词（滚），括号状态使用右下角小字显示并永久保存。
- 新增 7 节点滚动歌词，当前歌词居中高亮，上下歌词平滑滚动且只维护可见窗口，避免整首歌词一次性渲染。
- “双行歌词”调整为独立“显示译文”开关；网易云、QQ 与本地歌词译文支持时间戳容错对齐，同时间戳双语 LRC 可自动合并。
- 保留游客登录和本地播放能力，游客不会触发 QQ 或网易云自动登录弹窗，也能完整使用本地曲库与本地歌单。

## v1.1.2

- 新增游客登录：只需填写用户名，无需密码，游客身份保存在本机。
- 游客登录后不再自动弹出网易云或 QQ 音乐登录窗口；需要云端歌单时仍可在账号信息中手动补登。
- 新增“批量导入本地歌曲”：选择文件夹后递归扫描常见音乐文件，并自动匹配同名 `LRC`、`LYR`、`TXT` 歌词。
- 新增舞台歌词单行/双行显示开关，设置会随本地布局和用户视觉存档保存。
